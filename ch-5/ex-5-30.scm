...
(define (extend-environment vars vals base-env)
  (if (= (length vars) (length vals))
      ...
      (if (< (length vars) (length vals))
          'too-many-arguments-supplied-error 
         'too-few-arguments-supplied-error)))
...
(define (unbound-variable-error? x)
  (eq? x 'unbound-variable-error))

(define (unbound-set-variable-error? x) (eq? x 'unbound-set-variable-error))

(define (too-many-arguments-supplied-error? x) (eq? x 'too-many-arguments-supplied-error ))

(define (too-few-arguments-supplied-error? x) (eq? x 'too-few-arguments-supplied-error ))
...
(define (compiled-procedure-entry c-proc) (cadr c-proc)) (define (compiled-procedure-env c-proc) (caddr c-proc))
...
(define eceval-operations
  (list
   ...
   (list 'unbound-set-variable-error? unbound-set-variable-error?)
   (list 'too-few-arguments-supplied-error? too-few-arguments-supplied-error?)
...
compound-apply
  ...
  (test (op too-many-arguments-supplied-error?) (reg env))
  (branch (label too-many-arguments-supplied-error))
  (test (op too-few-arguments-supplied-error?) (reg env))
  (branch (label too-few-arguments-supplied-error))
  (assign unev (op procedure-body) (reg proc))
  (goto (label ev-sequence))
  too-many-arguments-supplied-error
  (assign val (const too-many-arguments-supplied-error))
  (goto (label signal-error))
  too-few-arguments-supplied-error
  (assign val (const too-few-arguments-supplied-error))
  (goto (label signal-error))
...
ev-assignment-1
  ...
  (test (op unbound-set-variable-error?) (reg val))
  (branch (label signal-error))
  (assign val (const ok))
  (goto (reg continue))
