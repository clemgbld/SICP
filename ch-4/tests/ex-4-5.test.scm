...
(check (equal? (list '(lambda (result) if result (cadr result) #f) 'assoc 'b '((a 1) (b 2))) (cond->if (list 'cond '((assoc b ((a 1) (b 2))) => cadr) '(else #f)))) "should be able to transform cond into if with the recipient syntax")
)
