...
(define (text-of-quotation exp env) (let ((text (cadr exp)))
    (if (pair? text)
       (eval (fold-right (lambda (curr acc) (list 'cons (make-quoted curr) acc)) '() text) env) 
        text)))
...
(define (cond->if exp) (expand-clauses (cond-clauses exp))) (define (expand-clauses clauses)
(if (null? clauses)
'false
...
(define primitive-procedures (list (list 'primitive-cons cons) (list 'car car) (list 'cdr cdr)  
 (list 'null? null?) (list '+ +) (list '* *) (list '= =) (list '- -) (list 'list list) (list 'display display) (list 'newline newline) (list 'eq? eq?)))
...
(define (lookup-proc-arg object key)
  (lookup-variable-value key (procedure-environment object)))

(define (build-object-to-print object depth len)
  (if (null? object) 
    '()
        (if (or (>= depth 10) (>= len 10)) 
         (list "...") 
          (if (compound-procedure? object) 
          (if (eq? (car (procedure-parameters object)) 'user-cons-arg) 
           (let ((first (lookup-proc-arg object 'x)))
            (if (evaluated-thunk? first)
             (let ((rest (lookup-proc-arg object 'y)))
                (cons 
                (build-object-to-print (force-it first) (+ depth 1) len) 
                (if (evaluated-thunk? rest)
                  (build-object-to-print  (force-it rest) depth (+ len 1))
                  (list "...")))) 
              (list "..."))) 
 ...
 (define (user-print object) 
   (display (build-object-to-print object 0 0))) 
