...
(list 'cons cons) (list 'null? null?) (list '+ +) (list 'map (primitive-implementation map))))
...
;Louis Reasoner program is crashing because map is represented 
; like that ('primitive proc) so we need to get the procedure
;of the primitive map by applying primitive implementation on it
; because map has been added to scheme as a primitive but it is not a real primitive like car and cdr